...
"""Unit-test suite for `pptx2.opc.package` module."""
...
from pptx2.opc.oxml import CT_Relationship, CT_Relationships
from pptx2.opc.package import (
    OpcPackage,
    Part,
    PartFactory,
    XmlPart,
...
from pptx2.opc.packuri import PACKAGE_URI, PackURI
...
class Describe_RelatableMixin:
    """Unit-test suite for `pptx2.opc.package._RelatableMixin`.
...
class DescribeOpcPackage:
    """Unit-test suite for `pptx2.opc.package.OpcPackage` objects."""
    ...
    def it_can_find_the_next_available_partname(self, request, ns, expected_n):
        ...
        PackURI_ = class_mock(
            request, "pptx2.opc.package.PackURI", return_value=PackURI(next_partname)
        )
        package = OpcPackage(None)
    ...
    def it_constructs_its_relationships_object_to_help(self, request, relationships_):
        _Relationships_ = class_mock(
            request, "pptx2.opc.package._Relationships", return_value=relationships_
        )
        package = OpcPackage(None)
...
class Describe_PackageLoader:
    """Unit-test suite for `pptx2.opc.package._PackageLoader` objects."""
...
class DescribePart:
    """Unit-test suite for `pptx2.opc.package.Part` objects."""
...
class DescribeXmlPart:
    """Unit-test suite for `pptx2.opc.package.XmlPart` objects."""

    def it_can_be_constructed_by_PartFactory(self, request):
        partname = PackURI("/ppt/slides/slide1.xml")
        element_ = element("p:sld")
        package_ = instance_mock(request, OpcPackage)
        parse_xml_ = function_mock(request, "pptx2.opc.package.parse_xml", return_value=element_)
    ...
    def it_can_serialize_to_xml(self, request):
        element_ = element("p:sld")
        serialize_part_xml_ = function_mock(request, "pptx2.opc.package.serialize_part_xml")
...
class DescribePartFactory:
    """Unit-test suite for `pptx2.opc.package.PartFactory` objects."""

    def it_constructs_custom_part_type_for_registered_content_types(self, request, package_, part_):
        SlidePart_ = class_mock(request, "pptx2.opc.package.XmlPart")
    ...
    def it_constructs_part_using_default_class_when_no_custom_registered(
        ...
        Part_ = class_mock(request, "pptx2.opc.package.Part")
...
class Describe_ContentTypeMap:
    """Unit-test suite for `pptx2.opc.package._ContentTypeMap` objects."""
...
class Describe_Relationships:
    """Unit-test suite for `pptx2.opc.package._Relationships` objects."""
    ...
    def _Relationship_(self, request):
        return class_mock(request, "pptx2.opc.package._Relationship")
...
class Describe_Relationship:
    """Unit-test suite for `pptx2.opc.package._Relationship` objects."""
