...
from pptx2.errors import RelationshipPolicyError, TargetNotFoundError, UnsupportedStructureError
...
class ImportReport:
    ...
    * ``dropped_placeholders`` -- names of furniture placeholders (dt/ftr/sldNum)
      removed under "bake".
    * ``run_shifts`` -- :class:`pptx2.rebind.RunShift` entries for every run whose
...
def append_deck(
    ...
    *,
    ...
    from pptx2.presentation import Presentation as _Presentation
...
def _validate_arguments(
    ...
    from pptx2.slide import SlideLayout as _SlideLayout
    ...
    from pptx2.slide import _require_layout_enrolled, _require_slide_enrolled
...
def _validate_mode_preparation(source_slide, mode, binding):
    ...
    from pptx2.rebind import _compute_mapping, _resolution_state
...
def _reconcile_copied_placeholders(new_slide_part, mode, prep):
    ...
    from pptx2.enum.text import MSO_TEXT_UNDERLINE_TYPE
        ...
        if shape_values is not None:
            ...
            for p_idx, r_idx, values in shape_values["runs"]:
                ...
                if "color_rgb" in values:
                    run.font.color.rgb = RGBColor.from_hex(values["color_rgb"])
