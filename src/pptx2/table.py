...
def _apply_cell_margins(cell: "_Cell", margin: "float | Length | Sequence[float | Length]") -> None:
    """Set a cell's four insets from a scalar or ``(top, right, bottom, left)``."""
    if isinstance(margin, (tuple, list)):
        if len(margin) != 4:
            raise ValueError(
                f"margin tuple must have 4 elements (top, right, bottom, left); got {len(margin)}"
            )
...
class Table(object):
    ...
    def delete_row(self, row_idx: int) -> None:
        ...
        if row_count == 1:
            raise ValueError("cannot delete the last remaining row; delete the table shape instead")
        conflicts = [
            region
            for region in self._merge_regions()
    ...
    def insert_column(
        ...
        *,
        ...
        if -1 < after < col_count - 1:
            conflicts = [
                region for region in self._merge_regions() if region.left <= after < region.right
            ]
            if conflicts:
        ...
        template_tcPrs = (
            tuple(
                copy.deepcopy(tr.tc_lst[copy_format_from].tcPr)
        ...
        new_width = (
            ...
            else Emu(self._tbl.tblGrid.gridCol_lst[default_width_idx].w)
        )
        from pptx2._transaction import PackageTransaction
    ...
    def insert_row(self, after: int, *, copy_format_from: int | None = None) -> _Row:
        ...
        if -1 < after < row_count - 1:
            conflicts = [
                region for region in self._merge_regions() if region.top <= after < region.bottom
            ]
            if conflicts:
                self._refuse_merge_conflict(
        ...
        template_idx = (
            copy_format_from if copy_format_from is not None else (0 if after == -1 else after)
        )
        template_tr = self._tbl.tr_lst[template_idx]
        template_tcPrs = (
            tuple(deepcopy(tc.tcPr) if tc.tcPr is not None else None for tc in template_tr.tc_lst)
            if copy_format_from is not None
            else None
        )
        from pptx2._transaction import PackageTransaction
    ...
    def fit_to_box(
        ...
        *,
        ...
        if min_font_pt <= 0 or max_font_pt < min_font_pt:
            raise ValueError("min_font_pt must be > 0 and max_font_pt must be >= min_font_pt")
    ...
    def _validate_grid_index(value: int, name: str, count: int, *, lower: int = 0) -> None:
        ...
        if isinstance(value, bool) or not isinstance(value, int) or not (lower <= value < count):
            raise ValueError(
